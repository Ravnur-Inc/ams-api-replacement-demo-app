...
Yes, you need to do so anyway because when the AMS streaming endpoint is deactivated, the AMS domain's DNS mapping to your CDN endpoint will also be removed. If you wish to continue using the AMS streaming domain, please contact AMS support. However, if you use your own custom domain, you don't need to change your streaming links. All you need in this case is to change is your CDN origin so that it uses the RMS streaming endpoint hostname instead of AMS. That's why we suggest you to create your own custom domain, map it to your existing CDN endpoint and change your streaming links in advance.
